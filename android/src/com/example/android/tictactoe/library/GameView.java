...
public class GameView extends View {
	...
	private final Bitmap mBmpPlayer2;
	private final Drawable mDrawableBg;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mWinRow >= 0) {
			...
			canvas.drawLine(x7 + MARGIN, y7 + MARGIN, x7 + s3 - 1 - MARGIN, y7 + s3 - 1 - MARGIN, mWinPaint);

		} else if (mWinDiag == 1) {
			// diagonal 1 is from (0,2) to (2,0)

			canvas.drawLine(x7 + MARGIN, y7 + s3 - 1 - MARGIN, x7 + s3 - 1 - MARGIN, y7 + MARGIN, mWinPaint);
		}
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (action == MotionEvent.ACTION_DOWN) {
			...
			if (isEnabled() && x >= 0 && x < 3 && y >= 0 & y < 3) {
				...
				mBlinkRect.set(MARGIN + x * sxy, MARGIN + y * sxy, MARGIN + (x + 1) * sxy, MARGIN + (y + 1) * sxy);
